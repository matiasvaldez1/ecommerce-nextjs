...
export default function Cart(){
  ...
  //Para solo mostrar algunos items en el desplegable
  const cartSliced = cartItems.slice(0,3)
    ...
    <div className="mr-12">
      ...
      {cartSliced && cartOpen  && (
        <div className="absolute top-12 right-20">
          <h2 className="text-black bg-gray-300 dark:bg-gray-500 rounded-sm pl-10 dark:text-white">Your Cart</h2>

          {cartSliced.length === 0 ? (
            <p>Your Cart Is Empty ☹</p>
          ) : (
            <div className="bg-gray-100 dark:bg-slate-400 dark:text-black">
              {cartSliced.map((item, i) => (
                <ItemCart key={i} item={item} />
              ))}
            </div>
          )}
        <div className="bg-green-500 dark:bg-slate-500 dark:border-2 dark:border-slate-800 rounded-sm text-white text-center hover:opacity-80">
