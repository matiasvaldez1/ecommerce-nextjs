...
export default function Component() {
  return (
    ...
    <div className="w-6/12 h-[400px] m-auto grid place-content-center">
      <button className="bg-slate-900 text-white p-2 rounded-md hover:opacity-70" onClick={() => signIn("github",{ callbackUrl: `http://localhost:3000/home` })}>Sign in with Github</button>
      <button  className="bg-cyan-600 text-white p-2 rounded-md hover:opacity-70 my-2"  onClick={() => signIn("google",{ callbackUrl: `http://localhost:3000/home` })}>Sign in with Google</button>
    </div>
    </>
  )
}